#!/bin/bash
# Boot orchestrator:
#   1. Log RTC signals (system time / hwclock / last_known_time / shutdown marker)
#   2. Detect RTC reset (>= 24h gap vs last_known_time) and recover
#   3. Restart redis/influxdb if time was recovered
#   4. Wait until redis/influxdb are ready
#   5. Start webserver (Type=notify, blocks until READY=1)
#   6. Start sv500A35
...
# ── Signal C: last known time from heartbeat ──────────────────────
if [ -f "$TIME_FILE" ]; then
...
# Correlate with shutdown kind for inferred reset trigger
