<template>
  ...
  /**
   * Diagnosis API 호출 (setSettings 또는 setProfile)
   */
  const callDiagnosis = async (mode, data) => {
    try {
      if (mode == 0) {
        ...
        return { result: true, data: diagnosisResponse.data };
      } else {
        ...
        return { result: true, data: advancedResponse.data };
      }
    } catch (error) {
  ...
  const transNumber = (channelData) => {
    ...
    // Boolean 필드를 0 또는 1로 변환
    const boolFields = ["Enable", "PowerQuality", "useDO", "confStatus", "useAI"];
    ...
    // confStatus가 false이면 status_Info 초기화
    ...
    if (data.ctInfo) {
      for (const key in data.ctInfo) {
        if (key === "direction") {
          data.ctInfo[key] = data.ctInfo.direction.map((d) => parseInt(d));
        } else if (key === "inorminal") {
          data.ctInfo[key] = parseFloat(data.ctInfo[key]);
        } else {
          data.ctInfo[key] = parseInt(data.ctInfo[key]);
        }
      }
    }
    ...
    if (data.ptInfo) {
      for (const key in data.ptInfo) {
        data.ptInfo[key] = parseInt(data.ptInfo[key]);
      }
    }
    ...
    if (data.eventInfo) {
      for (const key in data.eventInfo) {
        data.eventInfo[key] = parseInt(data.eventInfo[key]);
      }
    }
  ...
  /**
   * Nameplate 변경 확인 및 사용자 컨펌
   */
  const checkAndConfirmNameplateChanges = async () => {
    ...
    if (needsNameplateConfirmation) {
      ...
      if (!userConfirmed) {
        throw new Error("User cancelled nameplate changes");
      }
    }
  };
  
  /**
   * Nameplate 및 Asset Params 저장
   */
  const saveAssetConfigurations = async () => {
    for (const channelName in diagnosis_detail.value) {
      const channelData = diagnosis_detail.value[channelName];
      const isDiagnosisEnabledForChannel =
        channelName === "main"
      ...
      if (
        ...
        if (!nameplateResult.success) {
          alert(
            `❌ Failed to save ${channelName} channel nameplate settings:\n${nameplateResult.error}`
          );
          throw new Error(nameplateResult.error);
        }
      }
  ...
  /**
   * Diagnosis 설정 저장 통합 함수
   */
  const saveDiagnosisSettings = async () => {
    let errorMessages = [];
    ...
    // 1. setSettings API (DBMS)
    if (diagnosisData.value && Object.keys(diagnosisData.value).length > 0) {
      const ret = await callDiagnosis(0, diagnosisData.value);
      if (ret.result) {
        const { status, success, error } = ret.data;
        ...
        if (status === "0" || status === 0) {
          ...
          errorMessages.push(
            ...errorList.map((err) => `Diagnosis settings: ${err}`)
          );
        } else if ((status === "1" || status === 1) && !success) {
          const errorList = Array.isArray(error)
            ? error
            : [error || "Diagnosis settings save failed"];
          errorMessages.push(
            ...errorList.map((err) => `Diagnosis settings: ${err}`)
          );
        }
      } else {
        errorMessages.push(`Diagnosis settings: ${ret.error}`);
      }
    }
  
    // 2. setProfile API (Advanced Nameplate)
    if (advancedData.value && Object.keys(advancedData.value).length > 0) {
      const ret = await callDiagnosis(1, advancedData.value);
      if (ret.result) {
        const { status, success, error } = ret.data;
        ...
        if (status === "0" || status === 0) {
          ...
          errorMessages.push(`Check Smart API Service:`);
          errorMessages.push(
            ...errorList.map((err) => `Advanced diagnosis settings: ${err}`)
          );
        } else if ((status === "1" || status === 1) && !success) {
          const errorList = Array.isArray(error)
            ? error
            : [error || "Advanced diagnosis settings save failed"];
          errorMessages.push(
            ...errorList.map((err) => `Advanced diagnosis settings: ${err}`)
          );
        }
      } else {
        errorMessages.push(`Advanced diagnosis settings: ${ret.error}`);
      }
    }
  ...
  /**
   * device0 모드용 Trend Params 설정
   */
  const setDevice0TrendParams = () => {
    if (devMode.value !== "device0") return;
  ...
  const saveNew = async () => {
    try {
      ...
      // 1. 사전 검증
      if (!validateDiagnosisAssets()) {
        return;
      }
  
      // 2. Nameplate 변경 확인 및 저장
      if (useDiagnosis.value) {
        await checkAndConfirmNameplateChanges();
        await saveAssetConfigurations();
      }
  
      // 3. Diagnosis 설정 저장
      const diagnosisResult = await saveDiagnosisSettings();
      if (!diagnosisResult.success) {
        errorMessages.push(...diagnosisResult.errors);
      }
  
      // 4. device0 모드 설정
      setDevice0TrendParams();
      ...
      if (response.data && response.data.status === "1") {
        ...
        const errorMsg =
          response.data?.error || "Settings save failed";
      ...
      if (error.message !== "User cancelled nameplate changes") {
  ...
  const setAssetParams = async (paramData, assetName, channelName) => {
    try {
      ...
      provide("saveAllSettings", saveNew);
