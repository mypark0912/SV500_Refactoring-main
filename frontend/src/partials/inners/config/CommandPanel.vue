    ...
    <div class="p-6">
      ...
      <section class="mt-6 pb-6 border-b border-gray-200 dark:border-gray-700/60">
        <div class="grid grid-cols-12 gap-6">
          ...
          <div class="col-span-full xl:col-span-6 2xl:col-span-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 shadow-sm rounded-lg p-4">
            <div class="flex flex-col gap-5">
              
              <!-- Backup Download 섹션 -->
              <div class="flex items-center gap-3 pb-3 border-b border-gray-200 dark:border-gray-700/60">
                <label class="text-sm text-gray-700 dark:text-gray-300 font-medium whitespace-nowrap">
                  Backup Download
                </label>
              ...
              <div class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-3">
                  <label class="flex items-center cursor-pointer">
                    ...
                    />
                    <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Update</span>
                  </label>
                  ...
                  <label class="flex items-center cursor-pointer">
                    <input 
                      ...
                      class="w-4 h-4 text-violet-600 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 focus:ring-violet-500 dark:focus:ring-violet-600 focus:ring-2 disabled:opacity-50"
                    />
                    <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Reinstall</span>
                  </label>
                </div>
      ...
      <Teleport to="body">
        ...
        >
          <div v-if="showUpdateModal" class="fixed inset-0 z-50 overflow-hidden flex items-center justify-center px-4 sm:px-6" role="dialog">
            <!-- Background overlay -->
            <div class="fixed inset-0 bg-black bg-opacity-30 transition-opacity" @click="showUpdateModal = false"></div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-2xl relative max-w-md w-full mx-auto z-50 p-8">
              ...
              <div class="text-center mb-6">
                ...
                <p class="text-sm font-medium text-gray-900 dark:text-gray-100 mt-4">
    ...
    const performUpdate = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          showMessage(
            mode === 1 
              ? t('config.system.reinstall_success') || '시스템 재설치가 완료되었습니다.' 
              : t('config.system.update_success') || '시스템 업데이트가 완료되었습니다.'
          )
          ...
          showMessage(
            t('config.system.update_failed') || '업데이트 실패: ' + (response.data.message || '알 수 없는 오류')
          )
        }
      } catch (error) {
        console.error('Update failed:', error)
        showMessage(
          t('config.system.update_failed') || '업데이트 실패: ' + (error.response?.data?.message || error.message)
        )
      } finally {
