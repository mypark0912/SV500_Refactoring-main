    ...
    >
      ...
      <div class="px-4 py-4 space-y-3">
        ...
        <EN50160Chart
          ...
          chartHeight="300px"
          :showHistogram="true"
          :histogramData="frequencyHistogram"
          histogramUnit="Hz"
          histogramColor="#008080"
          :histogramLimitMin="59.4"
          :histogramLimitMax="60.6"
        />
        ...
        <EN50160Chart
          ...
          chartHeight="320px"
          :showPhaseHistograms="true"
          :phaseHistograms="voltagePhaseHistograms"
          histogramUnit="V"
          :histogramLimitMin="voltageLimitMin"
          :histogramLimitMax="voltageLimitMax"
        />
        ...
        <EN50160Chart
          ...
          chartHeight="280px"
          :showPhaseHistograms="true"
          :phaseHistograms="thdPhaseHistograms"
          histogramUnit="%"
          :histogramLimitValue="8"
        />
        ...
        <EN50160Chart
          ...
          chartHeight="250px"
          :showHistogram="true"
          :histogramData="unbalanceHistogram"
          histogramUnit="%"
          histogramColor="#9c27b0"
          :histogramLimitValue="2"
        />
...
import EN50160Harmonics from './EN50160Harmonics.vue'  // Harmonics 컴포넌트 추가
...
export default {
  ...
  setup(props) {
    ...
    const frequencyHistogram = computed(() => {
      return enReport_data.value?.frequency?.histogram || null
    })
    ...
    const voltagePhaseHistograms = computed(() => {
      ...
      return {
        L1: volt.phases.L1?.histogram,
        L2: volt.phases.L2?.histogram,
        L3: volt.phases.L3?.histogram
      }
    })

    const voltageLimitMin = computed(() => {
      return enReport_data.value?.voltage?.limits?.limit_95?.min || null
    })

    const voltageLimitMax = computed(() => {
      return enReport_data.value?.voltage?.limits?.limit_95?.max || null
    })
    ...
    const thdPhaseHistograms = computed(() => {
      ...
      return {
        L1: thd.phases.L1?.histogram,
        L2: thd.phases.L2?.histogram,
        L3: thd.phases.L3?.histogram
      }
    })
    ...
    // Harmonics
    // ============================================================
    const harmonicsData = computed(() => {
      return enReport_data.value?.harmonics || null
    })
    ...
    const unbalanceHistogram = computed(() => {
      return enReport_data.value?.unbalance?.histogram || null
    })
    ...
    const fetchReportData = async () => {
      loading.value = true
      try {
        const response = await axios.get(`/report/week/${channel.value}/${filename}`)
