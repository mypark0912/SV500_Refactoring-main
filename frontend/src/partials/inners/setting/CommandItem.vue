      ...
      <div v-if="item == 'Clear'" class="grow">
        <header class="flex items-center mb-4">
          ...
          >
            ...
            <!--//{{ t("config.system.title_1") }} Clear Command-->
          </h3>
        </header>
  ...
  setup(props, { emit }) {
    ...
    const serviceOp = async (cmd) => {
      try {
        const response = await axios.get(
          `/setting/SysService/${cmd}/${item.value}`
        );
    ...
    const command = async (cmdItem, command) => {
      try {
        ...
        if (response.data.success) {
          ...
          else message.value = `ITEM:${cmdItem} COMMAND:${command} !`;
        } else {
          message.value = `ITEM:${cmdItem} COMMAND:${command} CHANNEL:${props.channel} Failed: ${response.data.message}`;
        }
      } catch (error) {
        console.error(error);
        message.value = `ITEM:${cmdItem} COMMAND:${command} CHANNEL:${props.channel} Failed: ${error}`;
      }
      emit("service-done", message.value);
    };
    ...
    const confirmReset = () => {
      const confirmed = confirm(
        "All settings will be deleted and reset to default.\nDo you want to proceed?"
      );
      if (confirmed) {
        Reset();
      }
    ...
    const upload = async () => {
      ...
      try {
        ...
        if (response.data.passOK == "1") {
          ...
          console.log("GetSettingData type:", typeof GetSettingData);
          if (GetSettingData) {
            console.log("GetSettingData exists");
            try {
              await GetSettingData();
              message.value = "Upload and reload completed successfully!";
            } catch (reloadError) {
              console.error("설정 재로드 실패:", reloadError);
              message.value =
                "Upload Success, but failed to reload settings. Please refresh the page.";
            }
          } else {
            console.log("GetSettingData is undefined");
            message.value =
              "Upload Success, but could not reload settings automatically. Please refresh the page.";
          }
        } else {
        ...
        if (response.data.passOK === "1") {
          ...
          if (GetSettingData) {
            try {
              ...
              console.error("설정 재로드 실패:", reloadError);
              message.value =
                "Restore Success, but failed to reload settings. Please refresh the page.";
            }
          }
        } else {
