  ...
  setup(props) {
    ...
    const getBarColors = (value) => {
      const colorMap = {
        ...
        1: {
          bg: 'rgba(16, 185, 129, 0.8)',    // 녹색 - Normal
        ...
        3: {
          bg: 'rgba(249, 115, 22, 0.8)',    // 오렌지 - Error
          border: 'rgba(249, 115, 22, 1)',
        },
        4: {
          bg: 'rgba(239, 68, 68, 0.8)',     // 빨강 - Critical
    ...
    // Event 상태 라벨 (기존 번역 키 사용)
    const getEventStatusLabels = () => {
      return [
    ...
    watch(locale, () => {
      ...
      chart.options.scales.y.ticks.callback = (value) => {
        const textstr = getEventStatusLabels()
              ...
              grid: {
                ...
                color: (context) => {
                  const colors = [
                    ...
                    'rgba(16, 185, 129, 0.1)',  // Normal
                    'rgba(245, 158, 11, 0.1)',  // Warning
                    'rgba(249, 115, 22, 0.1)',  // Error
            ...
            tooltip: {
              ...
              callbacks: {
                ...
                label: (context) => {
                  ...
                  const statusLabels = getEventStatusLabels()
...
.modern-chart-container {
  ...
  @apply rounded-2xl border border-gray-200/50 dark:border-gray-700/50;
...
/* 업데이트 시간 스타일 */
.update-time {
  @apply text-xs text-gray-500 dark:text-gray-400 tabular-nums;
}
...
.loading-spinner {
  @apply w-8 h-8 border-3 border-orange-200 border-t-orange-600 rounded-full;
