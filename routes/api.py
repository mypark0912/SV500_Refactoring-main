...
async def get_state_legacy(data, channel):
    """기존 로직 (fallback용) - 새로운 API 응답 구조에 맞춤"""
    if not data or not isinstance(data, dict):
        return {"status": -1}
    ...
    for category in state_list:
        ...
        # 모든 값이 0이면 "No Data"
        if all(item["Status"] == 0 for item in status_items):
            retDict[category] = {"status": 0, "item": "All"}
            continue
        ...
        retDict[category] = {"status": highest_status, "item": item_label}
...
def get_interval(mode, channel):
    ...
    if mode == 'demand':
        ...
        if redis_state.client.hexists("Equipment","SamplingPeriod"):
            demandData = redis_state.client.hget("Equipment","SamplingPeriod")
            intervalDict = json.loads(demandData)
            return {"success": True, "data":intervalDict[chName]}
        else:
            return {"success": False}
