...
async def start_weekly_report(
    ...
    try:
        ...
        # 처음 생성 시 umask 로 그룹 쓰기 비트가 빠져 0755 가 된다.
        # core(root)/webserver(ntekadmin, root그룹) 가 공유하는 디렉토리이므로
        # setgid + 그룹 쓰기를 강제(소유자가 아니면 chmod 불가 → 무시).
        try:
            os.chmod(REPORT_WORK_DIR, 0o2775)
        except OSError:
            pass
