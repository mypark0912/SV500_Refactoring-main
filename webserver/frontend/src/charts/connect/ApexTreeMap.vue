...
export default {
  ...
  setup(props) {
    ...
    // 상태별 색상 매핑 (기존 바차트와 동일)
    const statusColors = {
      0: 'rgba(100, 116, 139, 0.8)', // Stop - Gray
      1: 'rgba(16, 185, 129, 0.8)',  // OK - Green
      2: 'rgba(234, 179, 8, 0.85)',  // Warning - Yellow (더 노랗게)
      3: 'rgba(249, 115, 22, 0.8)',  // Inspect - Orange
      4: 'rgba(239, 68, 68, 0.8)',   // Repair - Red
    }

    // 모드에 따른 타이틀 (기존 바차트와 동일)
    ...
    // 범례 아이템 (mode에 따라 다른 i18n 키 사용)
    const legendItems = computed(() => {
      if (props.mode === 'PowerQuality') {
        // PQ 모드
        return [
          { label: t('diagnosis.tabContext.pqfe0'), color: statusColors[0] },
          { label: t('diagnosis.tabContext.pqfe1'), color: statusColors[1] },
          { label: t('diagnosis.tabContext.pqfe2'), color: statusColors[2] },
          { label: t('diagnosis.tabContext.pqfe3'), color: statusColors[3] },
          { label: t('diagnosis.tabContext.pqfe4'), color: statusColors[4] },
        ]
      } else {
        // Diagnosis 모드
        return [
          { label: t('diagnosis.tabContext.st0'), color: statusColors[0] },
          { label: t('diagnosis.tabContext.st1'), color: statusColors[1] },
          { label: t('diagnosis.tabContext.st2'), color: statusColors[2] },
          { label: t('diagnosis.tabContext.st3'), color: statusColors[3] },
          { label: t('diagnosis.tabContext.st4'), color: statusColors[4] },
        ]
      }
    })

    // 차트 데이터 변환 (기존 바차트 데이터 구조에 맞춤)
    ...
    const chartOptions = computed(() => ({
      ...
      tooltip: {
        ...
        custom: function({ series, seriesIndex, dataPointIndex, w }) {
          ...
          const statusLabels = [
            'Stop', 'OK', 'Warning', 'Inspect', 'Repair'
          ]
