              ...
              >
                ...
                <div class="mt-3 w-full bg-amber-200 dark:bg-amber-800 rounded-full h-1.5 overflow-hidden">
                  <div
                    ...
                    :style="{
                      width:
                        downloadPhase === 'starting' ? '10%' :
                        downloadPhase === 'loading_input' ? '25%' :
                        downloadPhase === 'rendering' ? '70%' :
                        downloadPhase === 'done' ? '95%' :
                        downloadPhase === 'downloading' ? '100%' : '5%'
                    }"
                  ></div>
                </div>
              </div>
...
export default {
  ...
  setup(props) {
    ...
    const downloadPhase = ref(""); // 현재 진행 단계 (starting | loading_input | rendering | done | downloading)
    ...
    // === 통합 주간 리포트 다운로드 (비동기 / 별도 프로세스) ===
    // 흐름: start → status 폴링 → download
    const downloadWeeklyReport = async () => {
      ...
      try {
        ...
        const startResp = await axios.get(
          `/report/weeklyReport/start/${channelComputed.value}/${selectedReport.value}`,
          {
            params: { locale: locale.value, asset_name: chName },
            timeout: 30000,
          }
        );
        if (!startResp.data.success) {
          throw new Error(startResp.data.message || "리포트 생성 시작 실패");
        }
        const taskId = startResp.data.task_id;

        // 2) 상태 폴링 (1.5초 간격, 최대 20분)
        ...
        while (Date.now() - startedAt < MAX_WAIT_MS) {
          ...
          const statusResp = await axios.get(
            `/report/weeklyReport/status/${taskId}`,
            { timeout: 10000 }
          );
          if (!statusResp.data.success) {
            throw new Error(statusResp.data.message || "상태 조회 실패");
          }

          const { status, error, progress } = statusResp.data;
          if (progress && progress.phase) {
            downloadPhase.value = progress.phase;
          }
          ...
          if (status === "failed") {
            throw new Error(error || "리포트 생성 실패");
          }
        }

        if (!completed) {
          throw new Error(t("report.downloadTimeout") || "리포트 생성 시간이 초과되었습니다.");
        }

        // 3) 완료된 docx 다운로드
        downloadPhase.value = "downloading";
        const downloadResp = await axios.get(
          `/report/weeklyReport/download/${taskId}`,
    ...
    // phase 코드 → 한국어 라벨 매핑 (로딩 모달용)
    const phaseLabel = computed(() => {
      const map = {
        starting: t("report.modal.phaseStarting") || "시작 중...",
        loading_input: t("report.modal.phaseLoading") || "입력 데이터 읽는 중...",
        rendering: t("report.modal.phaseRendering") || "차트 및 문서 생성 중...",
        done: t("report.modal.phaseDone") || "완료",
        downloading: t("report.modal.phaseDownloading") || "파일 다운로드 중...",
      };
      return map[downloadPhase.value] || (t("report.modal.generating") || "리포트 생성 중...");
    });
