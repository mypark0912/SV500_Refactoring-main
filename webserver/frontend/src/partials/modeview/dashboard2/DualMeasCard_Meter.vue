        ...
        <div class="detail-block">
          ...
          <div class="phase-section">
            <div v-for="(phase, index) in ['L1', 'L2', 'L3']" :key="phase" class="phase-row">
              ...
              <div class="phase-bar">
                <div class="phase-fill" :class="`phase-fill-${index + 1}`"
                     :style="{ width: Math.min(getPhaseVoltage(index + 1) / 240 * 100, 100) + '%' }"></div>
              </div>
            </div>
          </div>
        </div>
...
import DashboardCard_THD from './DualMeasCard_THD.vue'

export default {
  name: 'DualMeasCard_Meter',
  components: { DashboardCard_THD },
  props: {
  ...
  setup(props) {
    ...
    const channelSetup = computed(() => {
      ...
      if (channel.value === 'Main') {
        if (setting.MainEnable && setting.MainDiagnosis) return 2
        else if (setting.MainEnable && !setting.MainDiagnosis) return 1
        else return 0
      } else {
        if (setting.SubEnable && setting.SubDiagnosis) return 2
        else if (setting.SubEnable && !setting.SubDiagnosis) return 1
        else return 0
      }
    })
...
.phase-value {
  @apply text-base font-bold tabular-nums min-w-[60px];
}
.phase-unit {
  @apply text-xs font-medium text-gray-600 dark:text-gray-400;
}
.phase-bar {
  @apply flex-1 h-2.5 bg-gray-200 dark:bg-gray-600 rounded-full overflow-hidden;
}
.phase-fill {
  @apply h-full rounded-full transition-all duration-500;
}
.phase-fill-1 { @apply bg-red-500; }
.phase-fill-2 { @apply bg-yellow-500; }
.phase-fill-3 { @apply bg-blue-500; }
