...
def restore_influxToken(org_id:str, token:str):
    ...
    try:
        ...
        # /etc/profile.d/influx.sh 생성 (root 소유 → sudo tee).
        # 로그인 셸이 source 하므로 기본 644면 충분(실행비트 불필요).
        content = f"""export INFLUX_TOKEN={token_escaped}
export INFLUX_HOST="http://localhost:8086"
export INFLUX_ORG={org_escaped}
export INFLUX_BUCKET=nteks
"""
        sudo_write_file("/etc/profile.d/influx.sh", content)
...
''',
        ...
        async with httpx.AsyncClient(timeout=setting_timeout) as client:
            ...
            #    422 가정만으로는 setup 재실행 때마다 같은 이름이 누적됨 → 선조회 후 없을 때만 생성.
            existing_names = set()
            try:
            ...
            for task_info in tasks:
                if task_info["name"] in existing_names:
                    logging.info(f"ℹ️ Task '{task_info['name']}' already exists (skip)")
...
def sudo_write_file(path, content, mode=None):
    """root 소유 경로에 sudo로 파일 생성.

    webserver는 ntekadmin(비root)으로 실행되므로 /etc/systemd/system,
    /usr/local/bin 등에 직접 open('w') 하면 PermissionError가 난다.
    sudoers에 경로 화이트리스트로 허용된 `tee` 와 `chmod` 만 경유한다.
    (광범위한 `bash -c` 규칙은 보안상 제거됨 — sudoers 참고)
    """
    subprocess.run(
        ['sudo', 'tee', path],
...
"""
    ...
    try:
        sudo_write_file(filename, service_content)
        return True
    except Exception as e:
        print(str(e))
        return False
